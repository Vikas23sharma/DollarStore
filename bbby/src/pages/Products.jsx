...
const Products = () => {
  ...
  const handleCategory = (children) => {
    setLoading(true)
    axios.get(`https://dummyjson.com/products/category/${children}`)
      .then((res) => {
        setLoading(false)
        console.log(res.data)
        setTotal(res.data.total)
  ...
  const handleSort = (value) => {
    if (value === "2") {
      const sortedProducts = brandproduct.sort((a, b) => { return a.price - b.price })
      setProducts(sortedProducts)
    }
    else if (value === "3") {

      const sortedProducts = brandproduct.sort((a, b) => { return b.price - a.price })
      setLoading(true)
      setProducts(sortedProducts)
      setLoading(false)
    }
